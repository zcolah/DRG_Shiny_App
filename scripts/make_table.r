...
MakeTable <- function(data, drg, payment){
  ...
  data <- data %>% 
    ...
    mutate(City = Provider.City, "Hospital Name" = Provider.Name, "Average Covered Charges ($)" = as.numeric(gsub("\\$", "", Average.Covered.Charges)), "Average Total Payments ($)" = Average.Total.Payments, "Average Medicare Payments ($)" = as.numeric(gsub("\\$", "", Average.Medicare.Payments))) %>%  # This will name the columns how I want it to
